...
- **Balance re-tune (AC5)**: FR9 global range buffed the ranged/caster archetypes; `three-mages` hit 70%+ and `gale`/`farshot` clipped the wipeout band. Re-tuned two placements (NOT stats): three-mages' knights pulled to the back (exposing the mid mages), gale's two archers pushed to the front. Converged rates (runs≥150) now top ~62.8% single / ~60% wipeout — genuinely in band. The tactic dimension (per-side committed tactics, FR24) adds variance, so the CI proxy stays at the pinned baseSeed=1/runs=15 (deterministic; a bigger sweep starves the parallel property tests' timeouts).
...
- AC2 ✅ — Placement tactic picker (4 beveled buttons, Autonomous default, **Attack Leader disabled** per D-3b, ≥44px), committed to `MatchSetup.tactics.A` via `MatchFlow.setTactic`; hidden until reveal. `playerTactic` survives army mutation (unlike the leader).
...
- AC5 ✅ — sweep sweeps tactics as a dimension; both-mode ≤65% band holds at the CI config; wardens single-mode floor 34.6% > the 3.0 33% baseline (asserted); rules.md + Help absorb tactics (drift guard green).
- **Gate green**: typecheck (both packages) clean, eslint + prettier clean, 230 engine + 186 web = **416 tests** pass, engine coverage **99.49% lines** (94.67% branch) — well above the AD-7 90% gate. NOTE: on a heavily-loaded machine the parallel engine run can flake a few fast-check property tests on their 5s timeout (pure CPU contention — all pass serially / with a raised timeout); real CI runners are unaffected.
- **Device sign-off PENDING** (Danilo, post-dev — as in 4.3): the picker feel, the disabled Attack Leader, the reveal labels, and a full loop where the tactic visibly changes targeting. 3 non-blocking open questions logged (AI-tactic window, picker layout, reveal-label placement).
...
- `src/flow/MatchFlow.ts` — `setTactic()` setter; `emptyState`/`startReplay` init; commit threads `playerTactic` (A) + `ai.tactic` (B).
- `src/scenes/PlacementScene.ts` — `buildTacticPicker()` (4 buttons, Attack Leader disabled).
- `src/scenes/RevealScene.ts` — both-tactic disclosure labels.
- `src/config/constants.ts` — `TACTIC_DISPLAY_NAME` / `TACTIC_SHORT_LABEL` / `TACTIC_BLURB` / `TACTIC_PICKER_HEADING`.
- `src/flow/draftModel.ts` — archer card behavior updated (global range, drops "reachable").
- `test/match-flow.test.ts` — tactic commit/setter/persistence tests.

- `src/scenes/DraftScene.ts` — double-tap a class tile to draft it (shares `addToArmy` with the button; Danilo's request).
