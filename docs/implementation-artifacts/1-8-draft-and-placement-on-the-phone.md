...
_Reviewed 2026-07-13 via `bmad-code-review` (Blind Hunter + Edge Case Hunter + Acceptance Auditor, diff 49808cf..HEAD incl. the UX polish, run on Opus 4.8 — same tier as the implementer, so cross-model independence was lower this pass; noted). Acceptance Auditor: all 6 ACs met, gate green (190 tests, typecheck clean), all scope fences respected — the only AC gap is the missing per-card sprite placeholder. 1 decision, 10 patches, ~10 dismissed as noise/by-design._

- [x] [Review][Defer] Navigation dead-ends: the flow is strictly one-way (Home→Draft→Placement→Reveal) with no back-nav, and the Reveal placeholder has NO exit — after commit the player is stranded until they reload the tab. DEFERRED (PO decision) to story 1.9, which owns the real post-submit screens (Reveal/Battle/Result) and navigation (Result→Rematch→Home); 1.8 remains a demoable one-way milestone [apps/web/src/scenes/*].
- [x] [Review][Patch] AC1/Task 2 gap: class cards render a background rect + text but no distinct "sprite PLACEHOLDER (colored rect + class initial/label)" as the AC enumerates. Add a per-card placeholder glyph [apps/web/src/scenes/DraftScene.ts].
- [x] [Review][Patch] `ELEMENT_COLORS` is typed `Record<'fire'|'water'|'wind'|'earth', number>` — it retypes the engine's `Element` union by hand, the exact "never retype engine data" lesson the diff itself cites. Use `Record<Element, number>` so a future element is a compile error, not a runtime `undefined` fill [apps/web/src/config/constants.ts].
- [x] [Review][Patch] Dead `cardSelected` palette entry — added this story, never referenced. Remove [apps/web/src/config/constants.ts].
- [x] [Review][Patch] Redundant `as Element` cast in `commit()` side-B roll (`rollElement` already returns `Element`); asymmetric with the un-cast side-A path. Remove the cast [apps/web/src/flow/MatchFlow.ts].
- [x] [Review][Patch] Dead, untested `unplaceUnit` method — no scene calls it (return-to-origin is redraw-based), and it has zero coverage. Remove (YAGNI; 1.9 adds a real tray-return path if needed) [apps/web/src/flow/MatchFlow.ts].
- [x] [Review][Patch] `commit()` has no double-invocation guard: a second call (e.g. a fast double-tap before the scene transitions) re-derives a DIFFERENT AI board because it now reads its own `lastAiArchetypeId` as `exclude`. Return the existing `committedSetup` if already committed — this is the one irreversible action [apps/web/src/flow/MatchFlow.ts].
- [x] [Review][Patch] `MatchFlow.placeUnit` doesn't bounds-check `unitIndex`, unlike its sibling `removeUnit`; a bad drag-data index would extend `playerPlacements` past `playerArmy` and desync `commit()`. Add the same guard [apps/web/src/flow/MatchFlow.ts].
- [x] [Review][Patch] The AD-5 phase FSM permits every transition: `draftUnit`/`removeUnit`/`placeUnit` have no phase guard, so a post-commit mutation would silently leave `committedSetup` describing a stale army (and `placeUnit` even regresses `phase` from `committed`). Unreachable in today's linear flow, but cheap to guard (throw once `committed`) before 1.9 adds re-entry [apps/web/src/flow/MatchFlow.ts].
- [x] [Review][Patch] Presentation layer hardcodes the army size `3` (draft slot loop, the `(n/3)` count label) where the model layer derives from `BALANCE.armySize` — reintroduces the drift risk the models avoid. Derive the loop/count from `BALANCE.armySize` (player-copy prose like "place all 3 units" can stay literal) [apps/web/src/scenes/DraftScene.ts].
- [x] [Review][Patch] The serializability test round-trips only a trivial state (2 units, no placements, no `committedSetup`). The actual 1.9 hand-off is the COMMITTED state carrying a nested `MatchSetup` — never round-tripped. Strengthen the test to serialize a committed state [apps/web/test/match-flow.test.ts].

_Follow-up Edge Case Hunter pass (2026-07-13) — 2 real defects, both fixed and pinned with regression tests:_

- [x] [Review][Patch] `commit()`'s incomplete-board error hardcoded the army size (`place all 3 units first`) where every sibling guard derives it from `BALANCE.armySize` — a drifted literal the moment army size changes. Interpolate `${BALANCE.armySize}`; pinned by asserting the message is derived from the constant [apps/web/src/flow/MatchFlow.ts, apps/web/test/match-flow.test.ts].
- [x] [Review][Patch] `removeUnit`/`placeUnit` bounds checks (`index < 0 || index >= len`) let a `NaN`/non-integer index through — NaN comparisons are all false, so the guard doesn't throw and the bad index then corrupts state via `splice`/array access. Add a `Number.isInteger(index)` guard to both; pinned by a test that feeds NaN/1.5 and asserts state is untouched [apps/web/src/flow/MatchFlow.ts, apps/web/test/match-flow.test.ts].

_Dismissed as noise / by-design (10): forward-only element on re-add has no player warning — that IS the ratified AC2 behavior (surfacing it is 2.x UX polish). `getState()` returns the live object — intentional; scenes re-fetch on every redraw, no consumer holds a stale ref. submit-gate vs `commit()`-guard predicate differ — `commit()`'s hard `=== armySize` check is a deliberate backstop; unreachable divergence. `init(data)` non-null assertion — reload always restarts at Home (scene[0]), which builds the flow. `rollNextElement` O(n²) stream replay — acknowledged in-comment, trivial at army size 3. `TEXT_RESOLUTION = 3` fixed vs `devicePixelRatio` — a documented fixed choice that resolves the blur; dpr-scaling is a micro-opt. `emptyState(0)` looks like a seed-0 match — Home always calls `startMatch` before Draft. `draftModel.beatenBy` uses `.find` (one attacker) — correct by construction for the RPS triangle. Grid cell labels "front/left" etc. — player-meaningful game terms (real rows/cols), aid orientation. `commit()` not wrapped in try/catch in the pointerup — it throws only on a deterministic assembly bug already covered by tests; freezing on that unreachable path is acceptable for the dev build. Input-listener teardown — a story-1.9 scene-re-entry concern, fine for a single scene lifetime._
...
- 2026-07-13: Code review (Opus 4.8; 1 decision, 10 patches, ~10 dismissed). Acceptance Auditor: all 6 ACs met, gate green, scope fences respected. Decision: navigation dead-ends (one-way flow + exit-less Reveal stub) deferred to story 1.9, which owns post-submit screens + navigation (logged in deferred-work.md). Patches applied: added the missing per-card sprite placeholder (AC1 gap); typed ELEMENT_COLORS as Record<Element, number> (the data-must-be-read lesson); removed dead cardSelected + dead/untested unplaceUnit + a redundant `as Element` cast; guarded commit() against double-invocation (would else re-derive a different AI board), bounds-checked placeUnit, and added post-commit phase guards on draft/remove/place; derived the draft army-size UI from BALANCE.armySize; strengthened the serializability test to round-trip a COMMITTED state with its nested MatchSetup. 194 tests green (4 added), typecheck + build clean. Story → done.
