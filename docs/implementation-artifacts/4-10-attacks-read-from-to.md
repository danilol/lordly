...
Senior code review 2026-07-20 (Fable 5, 3-layer: Blind Hunter + Edge Case Hunter + Acceptance Auditor). No High findings — the `eventTrace` seam is correct and complete (all 16 union members accounted for), the melee local-coordinate math is right, and the step fits every beat. 2 decisions, 6 patches, 0 deferred, 4 dismissed.

- [x] [Review][Decision] "On arrival" sequencing is prose, not code — the blast wash / heal glow / popups all fire at beat start, ~180ms BEFORE the trace sliver lands; three comments claim they're "arrival" effects. Options: (a) keep the shipped look (accepted on device) and fix the prose, or (b) delay the arrival effects by the trace duration for a stronger cause→effect read (changes what was accepted).
- [x] [Review][Decision] Guard-block flash target was never actually ruled on — AC2 and the engine's own `types.ts:287-289` comment say the shell flashes the GUARDIAN; shipped 4.7 behavior (kept in this story) flashes the shielded target. The dev flag ("ask at the device pass") was never answered. Options: (a) keep target-only + amend the AC prose, (b) flash the guardian too (one-line add), (c) guardian only.
- [x] [Review][Patch] Melee step has no absolute cap — a Cleric staff-bonk uses RANGED targeting (`resolve.ts:326-330`), so a `staff` step can travel 0.6× a cross-board vector (~175px vs the ~70px clash steps accepted on device); same for misfire strikes on distant allies. Add a `MELEE_STEP_MAX_PX` cap [apps/web/src/scenes/BattleScene.ts]
- [x] [Review][Patch] Self-targeted trace renders a degenerate zero-length sliver hovering on the unit for 180ms (self-heal — the Cleric can be its own lowest-HP ally; misfired mage self-blast with itself as `targets[0]`). Skip the projectile when from === to [apps/web/src/scenes/BattleScene.ts]
- [x] [Review][Patch] A heal/spell whose target view is missing falls into the melee ATTACK fallback — a healer would step toward the enemy board, a fabricated aggression read. Only attacks should use the fallback step [apps/web/src/scenes/BattleScene.ts:500-502]
- [x] [Review][Patch] Drift guards missing: the melee check is a stringly non-exhaustive `kind === 'slash' || 'bash' || 'staff'` and `traceMove` re-discriminates `event.type` with a silent `default` — a future `MoveKind`/sourced event silently misrenders; `bash`/`staff` have no `eventTrace` test fixtures; `deferred-work.md:143`'s "guard-full would render as a plain melee hit" claim is now stale (it would render as a projectile) [apps/web/src/scenes/BattleScene.ts + test + deferred-work.md]
- [x] [Review][Patch] Doc contradictions: the perf-verdict 4.10 addendum still says its table "is filled from that pass (Task 7 device pass)" but Task 7 closed without it (capture is post-deploy per deferred-work.md — which should also note a local `vite preview` capture was possible, only deployed-URL comparability requires the merge); the story File List omits `deferred-work.md` and `sprint-status.yaml` [docs/performance-verdict.md + this file]
- [x] [Review][Patch] Color/comment hygiene: `HEAL_TRACE_COLOR` is hardcoded in the scene against the one-source convention (`config/constants.ts`); its comment ("side hues never a travel line") contradicts the blast sliver using `actorFill` ten lines later — clarify the actual rule (attack effects follow the actor-color combat-number rule; heal/spell traces stay off side hues); `parseInt(STATUS_COLORS[…].slice(1),16)` silently yields NaN on a future non-`#rrggbb` token — pin the format with a test; `resetSprite`'s docstring still cites the retired "lunge 280ms"/"150ms beat" numbers [apps/web/src/scenes/BattleScene.ts + config/constants.ts]

Dismissed (4): blast traces to `targets[0]` only (the story's own recorded open-Q2 default, device-accepted); trace duration not speed-scaled (explicitly offered at the tuning round, Danilo shipped as-is); iso depth not re-sorted mid-step (device-accepted look; the sprite returns to rest each beat); mid-beat speed-toggle tween overlap (the existing `resetSprite` pattern already owns outliving tweens — self-correcting).
...
- `apps/web/src/flow/battleView.ts` — new `eventTrace(event)` pure seam + `EventTrace`/`TraceKind` types (the from→to derivation); review: `EventTrace` is now a discriminated union carrying `spell: SpellKind` on a `'spell'` trace.
- `apps/web/src/scenes/BattleScene.ts` — replaced `attackFlavor` with `traceMove(event)` (reads `eventTrace`) + new `meleeStep`/`traceProjectile`/`blastWash`/`afterTravel` helpers; melee now a real step-to-target-and-return; heal/spell render cases now trace; tuning constants (`MELEE_STEP_FRACTION`, `MELEE_STEP_FRACTION_REDUCED`, `MELEE_STEP_MS`, `MELEE_STEP_MAX_PX`, `MELEE_STEP_FALLBACK_PX`, `TRACE_MS`, `TRACE_MS_REDUCED`, exhaustive `TRACE_TRAVEL` map); removed dead `LUNGE_PX`. Review round: impact effects delayed to trace arrival; guard ring moved to the GUARDIAN; melee step capped (`MELEE_STEP_MAX_PX`); self-target trace skipped; heal/spell no longer use the attack fallback nudge; exhaustive `TraceKind` switch (no silent default).
- `apps/web/src/config/constants.ts` — review: `HEAL_TRACE_COLOR` + `statusTraceColor(spell)` moved here (one-source color convention), with the trace-color rule stated once.
- `apps/web/src/config/sprites.ts` — review: `UNIT_TWEENS.attack` doc comment updated (4.10's melee step supersedes its `props`/`duration`; only `repeat` is consumed).
- `apps/web/test/battle-view.test.ts` — `eventTrace` describe block (9 tests: attack/blast-fanout/guarded-no-retarget/heal/self-heal/spell-with-SpellKind/bash+staff + all 13 origin-less events → null).
- `apps/web/test/constants.test.ts` — review: trace-color format pins (`STATUS_COLORS` strict `#rrggbb` → `statusTraceColor` never NaN; `HEAL_TRACE_COLOR` valid + not a side hue).
- `docs/performance-verdict.md` — story-4.10 addendum: busiest-case decision + per-beat object accounting + deferred-capture table stub; review: stale "pending Task 7" wording fixed, arrival-delay zero-object note added.
- `docs/implementation-artifacts/deferred-work.md` — the 4.10 post-deploy `?perf=1` capture entry (review: `vite preview` comparability note added); review: 4.7 entry's stale "plain melee hit" claim updated for the exhaustive travel map.
- `docs/implementation-artifacts/sprint-status.yaml` — 4-10 status transitions.
...
- 2026-07-20 — Senior code review (Fable 5, 3-layer: Blind Hunter + Edge Case Hunter + Acceptance Auditor). No High findings — the seam verified correct and complete. 2 decisions (both resolved by Danilo: impact effects now land ON ARRIVAL of the trace, not at launch; the Guard ring moved to the GUARDIAN — AC2/types.ts contract, supersedes 4.7's target-side ring) + 6 patches applied (melee step capped at `MELEE_STEP_MAX_PX=90` — a ranged-targeted staff bonk/misfire could dash near-full-board; self-target traces skipped; heal/spell no longer fall into the attack nudge; exhaustive `TRACE_TRAVEL`/`TraceKind` guards + `EventTrace` carries `SpellKind` + bash/staff/self-heal fixtures; perf-doc/File-List/deferred-work contradictions fixed; trace colors moved to constants with a `#rrggbb` format pin + stale docstrings rewritten) + 4 dismissed. Gate re-run GREEN: 550 tests, typecheck+lint+prettier clean, engine cov 99.42% lines, web build succeeds. Status → done. NOTE for the deployed-build look: the arrival-delay + guardian-ring changes are review-decided but not yet seen on device — worth a glance after deploy, alongside the deferred `?perf=1` capture.
