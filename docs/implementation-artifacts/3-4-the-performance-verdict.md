...
**Open action from the review decisions — CLOSED (2026-07-16, same day):** Danilo ran the re-capture session on his Pixel 9 Pro XL with the deployed patched sampler (`?perf=1`, console armed-readout confirmed): Battle 1× (3,685 samples, min 40 — a single scene-entry frame — median 59.88, zero frames <30), Battle ×2 (1,279 samples, same shape), Placement with ~70s of dragging (4,416 samples, worst single frame 30.03fps during a touch drag, zero below the floor). 9,380 per-frame samples total, zero 30fps-floor breaches. Method + full tables in `docs/performance-verdict.md`. **AC1 closed on sound evidence.**
...
- 2026-07-16 (re-capture): **AC1 CLOSED on sound evidence.** Patched sampler deployed (commit 5cb9517, CI green), Danilo re-ran the session on his Pixel 9 Pro XL: Battle 1×, Battle ×2, and Placement (~70s of drag), with per-scenario `window.__perfSamples` resets. 9,380 per-frame instantaneous-fps samples, all three scenarios median ≈59.9 (vsync-quantized 60), worst single frame anywhere 30.03fps (one Placement drag frame — still above the floor), **zero frames below the 30fps floor**. Sample counts/durations internally coherent this time, all within one read of the 36,000 cap. Authoritative tables + artifact reading (scene-entry frame, 120Hz fast-frame samples) recorded in `docs/performance-verdict.md`. All three ACs satisfied.
- 2026-07-16: **DONE.** Danilo's sign-off: "let's flip it to done and move forward" — the recorded verdict stands. Epic 3's final story closes: the MVP's performance requirement (NFR1) is now measured and documented, not assumed.
