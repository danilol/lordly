...
- **Attribution:** the Golem is Danilo's original art, so it gets its OWN `ART_ATTRIBUTIONS` entry (author "Danilo Lima", **CC-BY-4.0** default) — removed from the DCSS pack. Credits auto-lists it. **OPEN:** confirm CC-BY-4.0 vs CC0 (both pass the FR31 gate; one-line change).
