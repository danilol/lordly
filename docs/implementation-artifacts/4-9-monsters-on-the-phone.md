...
- [x] **Task 1 — Source & composite a dedicated Golem sprite (AC: 4)**
  - [x] Tile chosen: **Danilo's own original art** (`newgolem.png`, a stone-and-moss construct with cyan eyes/core) — not the DCSS default. Side-neutral (stone/green), so it's safe on both sides.
  - [x] Composited into `apps/web/src/assets/units.png` as frame 6 (sheet 192×32 → 224×32); frames 0–5 unchanged. Processed with `sharp`: keyed out the solid near-white background → transparent, cropped to content, downscaled 1254px → 32×32. Bundle +1.8KB.
  - [x] `sprites.ts`: `UNIT_FRAMES.golem` 0 → 6; header + entry comments rewritten (no longer interim; seven frames, two sources). `BootScene` frame guard passes at 7.
  - [x] `attribution.ts`: removed the Golem from the DCSS pack; added its **own** manifest entry (Danilo Lima, CC-BY-4.0 by default — see open question) so Credits auto-lists it. No INTERIM note remains.
...
- [x] **Task 5 — Tests (AC: 1–4)**
  - [x] `sprites.test.ts` — bumped `SHEET_FRAMES` 6 → 7; asserts all seven dedicated frames are distinct and `UNIT_FRAMES.golem !== UNIT_FRAMES.knight` (no longer an interim share).
  - [x] `attribution.test.ts` — new test: the Golem traces to a supplier pack with **no** `(INTERIM…)` marker.
...
### Review Findings (senior code review, 2026-07-20 — 3-layer adversarial: Blind Hunter, Edge Case Hunter, Acceptance Auditor)

- [x] [Review][Decision] The 1.5× loom creates sprite/badge/card overlaps in 6 places beyond the one (Battle) already disclosed — Draft army tray, Placement unit card, History card (also overflows 4px above its own card), Reveal board label, Result composition chip (also overflows its chip), plus ResultScene was never in the story's scene checklist at all. Z-order keeps text/badges legible on top in every case, so this reads as the intended "loom overhang," not garbled UI. **RESOLVED — Danilo: accept everywhere, matching the Battle precedent, no code change.** The earlier claim "verified analytically... no container OVERFLOW anywhere" is corrected: History and Result DO overflow their own bounds by 3–4px, by design, accepted.
- [x] [Review][Patch] Draft DETAIL panel's role/slot-cost line overflows its ~170px wrap width for nearly every class (the new "· N slot(s) ·" segment adds ~12–14 chars to a line that was already near the wrap boundary), colliding with the fixed-position line 18px below [apps/web/src/scenes/DraftScene.ts:211] — fixed: the cost segment only shows for `slotCost > 1` (removes the regression for all 11 small classes entirely, reverting their line's length below the original — already-tight — budget), separators tightened. The Golem's own line is shorter than before but still close to the wrap edge; worth a glance next device visit (this codebase confirms layout on device, not by pixel measurement).
- [x] [Review][Patch] Story Dev Notes still frame the Golem's art license as "OPEN: confirm CC-BY-4.0 vs CC0" — Danilo already confirmed keeping CC-BY-4.0 in this session [docs/implementation-artifacts/4-9-monsters-on-the-phone.md:148] — fixed: reworded to state the resolution.
- [x] [Review][Patch] Two engine comments predating this story still describe the Golem as having a "two-cell footprint"/"two-cell body" — stale since story 4.8's single-cell revision; outside Task 4's stated `apps/web/src`-only scope but the same cleanup, worth closing now [packages/engine/src/balance.ts:314-315, packages/engine/src/types.ts:18] — fixed: both rewritten to the single-cell king-move wording.
- [x] [Review][Defer] The DCSS pack's PWA icon assets still trace to `vault_guard.png`, the tile the Golem no longer uses [apps/web/src/assets/attribution.ts] — deferred, pre-existing (icon generation untouched by this diff; a documentation nuance, not a functional issue)
...
- **Art LANDED (Task 1 done, same session).** Danilo made his own golem art (`newgolem.png`) instead of using the DCSS default. Processed with `sharp` (available as a transitive dep): keyed out the solid near-white background → transparent, cropped to the content bbox, downscaled 1254px → a crisp 32×32 tile, composited into `units.png` as frame 6 (sheet 192→224px, +1.8KB). `UNIT_FRAMES.golem` 0 → 6. It renders as a real stone-and-moss golem now, not a big knight. The 3 previously-parked tests are now active and green.
- **Attribution:** the Golem is Danilo's original art, so it gets its OWN `ART_ATTRIBUTIONS` entry (author "Danilo Lima", **CC-BY-4.0**) — removed from the DCSS pack. Credits auto-lists it. **RESOLVED:** Danilo confirmed keeping CC-BY-4.0 (not CC0) during this story's dev session.
- **Binding-UX token amended.** DESIGN `{components.golem-body}` + EXPERIENCE "Epic 4 extension" rewritten (dated 2026-07-20 notes) to the single-cell loom — "spans both cells" → "one tile, drawn large, overhangs the reserved ring"; the load-bearing invariants (≥48px, one HP bar, one code, one unit, FR39f) preserved. The spec now matches the shipped app.
- **STILL NOT device-verified (Task 6, Danilo's gate).** The loom size, the Battle code/badge anchor overlaps under the bigger sprite, placement "no drop lost", and the NFR1 frame rate on Android all still need the phone. Story stays **in-progress** until that device pass — **code + docs are complete; only the on-device Android acceptance remains.**

### File List

**Web (`apps/web`)**
- `src/assets/units.png` — sprite sheet grown 192×32 → 224×32; frame 6 is the Golem (Danilo's art, keyed + downscaled via `sharp`).
- `src/assets/attribution.ts` — new "Lordly original sprites" manifest entry (Danilo Lima, CC-BY-4.0) for the Golem; removed the Golem from the DCSS entry.
- `src/config/sprites.ts` — `UNIT_FRAMES.golem` 0 → 6; header + entry comments rewritten (seven frames, two sources; no longer interim).
- `test/sprites.test.ts` — `SHEET_FRAMES` 6 → 7; asserts seven distinct dedicated frames + Golem ≠ Knight.
- `test/attribution.test.ts` — new test: Golem traces to a supplier with no INTERIM marker.
...
- `docs/planning-artifacts/ux-designs/…/DESIGN.md` + `EXPERIENCE.md` — golem-body token amended (dated) to the single-cell loom.
- `.gitignore` — ignore `/art-src/` (high-res art masters kept on disk, not shipped).
...
- 2026-07-20 — **SENIOR CODE REVIEW (Opus 4.8, 3-layer adversarial: Blind Hunter, Edge Case Hunter, Acceptance Auditor) — DONE.** Triage: 1 decision-needed (the 1.5× loom's sprite/badge/card overlaps across 6 scenes beyond Battle — Danilo: accept everywhere, no code change), 3 patches (Draft DETAIL text-wrap regression fixed — cost segment now only shows for `slotCost > 1`; license doc corrected from "OPEN" to the already-confirmed CC-BY-4.0; two stale engine comments rewritten from "two-cell" to the single-cell king-move model), 1 deferred (DCSS pack's PWA icons still cite the old shared tile — pre-existing), 5 dismissed. Gate re-confirmed GREEN post-patch: 540 tests, typecheck/lint/prettier clean, engine coverage 97.77% lines, web build succeeds. Story done.
