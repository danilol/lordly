...
- **FR10. Row blast (Mage).** Each cast strikes **every unit in one enemy row**, ignoring column reach. Target row = the row with the most living enemies; tie broken toward the **rearmost** row. Damage applies to each unit in the row, with RPS modifiers per target. *Amended 2026-07-14 (sprint-change-proposal-2026-07-14):* each target's damage is **attenuated by the named balance ratio `blastAttenuation` (initial ×0.75)**, applied after the base formula and before RPS — the blast trades per-target power for its unmatched coverage. `[ASSUMPTION: ×0.75 initial value — a balancing lever, verified by the NFR4 sweep]` *OB64-style Archmage gating (base Mage single-target; blast unlocked by promotion) is explicitly parked as an Epic 4 design item (Feature 6b).*
