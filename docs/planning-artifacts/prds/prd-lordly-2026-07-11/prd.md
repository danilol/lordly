...
*Widened 2026-07-14 (sprint-change-proposal-2026-07-14).* Epic 4 is the **combat-depth design pass** — one PM/Architect scoping pass, one combined `logVersion`/`balanceVersion` bump — indexing four sibling systems that all change "what a unit does with its action": **position-dependent move variety** (FR32/FR33 below); **target-selection tactics** (OB64's attack-weakest / attack-strongest / autonomous — a new player-facing choice touching FR8/FR9, `MatchSetup`, and AD-9 determinism); **critical hits** (DEX finally spends its reserved purpose, FR15 — a crit chance/multiplier and a `crit` flag on the attack event payload); and **Archmage-style promotion gating** of the Mage's row blast (FR10's parked design item). The deferred `StatusCleared` events from the story-2.2 review ride the same single bump.
