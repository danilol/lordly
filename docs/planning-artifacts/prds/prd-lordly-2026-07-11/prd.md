...
- **FR14. The RPS triangle — and the archer's caster-hunter rule.** **Mage beats Knight, Knight beats Archer, Archer beats Mage.** An attack with class advantage deals **×1.5** damage; with disadvantage, **×0.75**. *Amended 2026-07-14 (sprint-change-proposal-2026-07-14):* additionally, the **Archer deals ×1.5 to every caster — Mage, Cleric, and Witch**. This is **one-way**: Cleric and Witch take no symmetric ×0.75 penalty when attacking the Archer (the Mage's disadvantage already exists via the triangle, which is unchanged). Mercenary sits fully outside (×1.0 both ways). Any change to these multipliers mandates NFR4 sweep re-verification (≤65% dominance band, both modes). `[ASSUMPTION: 1.5/0.75 multipliers — first balancing lever]`
...
### Feature 6b — Combat depth: moves, tactics, crits, promotions (post-MVP, Epic 4)

*Widened 2026-07-14 (sprint-change-proposal-2026-07-14).* Epic 4 is the **combat-depth design pass** — one PM/Architect scoping pass, one combined `logVersion`/`balanceVersion` bump — indexing four sibling systems that all change "what a unit does with its action": **position-dependent move variety** (FR32/FR33 below); **target-selection tactics** (OB64's attack-weakest / attack-strongest / autonomous — a new player-facing choice touching FR8/FR9, `MatchSetup`, and AD-9 determinism); **critical hits and miss/dodge** (DEX finally spends its full reserved purpose — "accuracy, evasion, and crits", FR15 — a crit chance/multiplier plus an accuracy/evasion roll, with `crit` and `missed`/`dodged` outcomes on the attack event payload); and **Archmage-style promotion gating** of the Mage's row blast (FR10's parked design item). The deferred `StatusCleared` events from the story-2.2 review ride the same single bump. *Determinism constraint (added 2026-07-15 from the story-3.2 replay review):* crit and hit/miss each consume a per-attack seeded `battle`-stream draw, so the design must fix their relative draw ORDER — every future seed depends on it (FR20). This is precisely what lets replay (story 3.2) reproduce crits and misses with zero per-hit storage: the seed is the recording, not the outcome.
