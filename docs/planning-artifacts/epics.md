...
FR10: Mage row blast — hits every unit in one enemy row, ignoring reach; row with most living enemies, tie → rearmost; per-target `blastAttenuation` ×0.75 after base, before RPS, in WIPEOUT mode only (amended 2026-07-14; mode-scoped in story 3.0 per the both-mode sweep, PO-approved); RPS per target. Archmage-style promotion gating of the blast: **postponed until after link-play** (PO decision at the Epic 4 breakdown, 2026-07-16 — promotion needs most/all classes present first; this era works with first-level classes only, closing PRD Open Item 4). *(Amended 2026-08-01, PO-ratified at the 5.4 engine review: story 5.4's dossier decision **E5-D4** RETIRED the row blast from the roster — the Wizard and Sorceress lost their splash and fire the single-target magic `bolt` from mid/back, so **no class carries a `blast` row today**. The rule above is not repealed: the `blast` kind, its targeting (fullest row, tie rearmost) and its wipeout `blastAttenuation` all stay live, data-driven engine code, held as RESERVED data for the future Archmage tier — any class given a `blast` row blasts exactly as specified here. What changed is the roster, not the requirement. Two consequences worth reading with it: the cross-engagement attenuation pin (4.12) has no in-play blaster left and was succeeded by the bolt's no-attenuation pin (`wipeout.test.ts`), and the row-AoE that satisfies FR25's anti-front-stack acceptance is now the dragons' `breath` (5.5, E5-D7), not the mage's blast.)*
...
FR25: AI selects from a curated strategy pool with seeded variation; punishes lazy formations; single difficulty targeting ~50% win rate. *(Amended 2026-07-29, PO-ratified: originally "~8–12 archetypes" — the pool grew to 18 with story 5.5's monster wave, an arithmetic consequence of the 4.12 reverse-coverage guard (every class must appear in some archetype) × 2-slot monsters (a monster comp holds only 3–4 units). The pool size is pinned exactly in `ai.test.ts`; further growth is a deliberate edit there, and the ≤65% sweep band remains the real curation gate. The story-1.7 AC below keeps its original 8–12 wording as the historical MVP acceptance.)*
