...
| E5-D16 | **A physical hit WAKES a sleeping unit; magic does not.** Adopted from OB64, with one deliberate simplification: the wake is GUARANTEED, not the sources' probabilistic "may wake up". Deterministic → no new draw → ADR 0003 stays frozen, and it errs toward the player on a status the PO already finds oppressive. Emits `StatusCleared { spell: 'sleep' }` so the shell narrates it. Carrier: 5.12 | **DECIDED** | 2026-08-01 | Danilo: "2 yes". Corroborated by two independent sources (sleepers "wake up fairly often, particularly when being attacked"; paralysis is rated stronger precisely because attacking does NOT cure it) |
| E5-D17 | **A deterministic 2-missed-action cap replaces OB64's `Base% + MEN/4` wake roll.** Sleep clears after the sleeper has missed **2** actions (tunable balance data, sweep-policed). The probabilistic roll is NOT adopted: a new per-turn draw would require formally amending ADR 0003's frozen draw table, which every stored replay depends on. Source-supported rather than invented — OB64 itself force-clears Sleep/Paralysis after ~3–5 missed actions in its endless loop to prevent soft-locks. Carrier: 5.12 | **DECIDED** | 2026-08-01 | Danilo: "2 yes; 3 yes" (cap value 2 confirmed) |
| E5-D18 | **Poison's tick timing and scale are NOT changed.** OB64 ticks poison pre-action at ~10–15% of Max HP; we keep our flat `poisonDamage` at engagement end. Adopting the OB64 model would make poison scale with HP (punishing monsters far harder than smalls) and multiply its frequency — a large swing the PO did not ask for. Recorded as a **known, dated deviation**, routed to `deferred-work.md` as a candidate for a later fidelity pass | **DECIDED** | 2026-08-01 | Danilo: "2 yes". Poison was not among the five reported problems |
| E5-D19 | **The Archer moves to `actions { front: 1, mid: 1, back: 2 }` as a DELIBERATE BALANCE DEVIATION, explicitly NOT as OB64 fidelity.** The independent research pass CONTRADICTED the fidelity rationale — sources state the OB64 Archer's pattern is "Shoot ×1 front, ×2 mid, ×2 back", i.e. exactly the `1/2/2` already shipped. The change is kept anyway on the PO's felt-play judgement (the archer half of the pool — longbows/talons/farshot/gale — reads as over-rewarded from the mid row). Recorded here so no future reader mistakes it for a source-driven fix. Carrier: 5.11 | **DECIDED** | 2026-08-01 | Danilo: "1 keep it", told that sources contradict the fidelity claim |
| E5-D20 | **Ranged Autonomous depth order front → middle → back is PO DIRECTION, not sourced fidelity.** No source found states a row preference for ranged attackers; the guides detail column sectors and per-row action counts but are silent here. Kept because it makes front-line screening meaningful for arrows (the strategic depth the PO is after) and because it matches the reported felt problem. The dossier must NOT claim OB64 mandates it. Carrier: 5.11 | **DECIDED** | 2026-08-01 | Danilo's felt-play report + his supplied research; independent verification found no corroborating source, recorded in §8 |
...
- Max-monsters (2) CONFIRMED as shipped, plus the new humans-only leader rule (E5-D13 — the Whelp doesn't count toward the monster cap, and no monster nor the Whelp can be crowned). The "never-same-column" clause this line once carried was WRONG and never shipped (corrected 2026-07-28 at 5.5 review, with ROSTER.md): the only placement rule is the king-move ring — front-left + back-left is legal.
...
## §8 OB64 status-ailment semantics (Sitting 5 — the story-5.12 design gate) — evidence 2026-08-01, **decisions RATIFIED 2026-08-01** (E5-D16/D17/D18; see also E5-D19/D20 in the decision table)
