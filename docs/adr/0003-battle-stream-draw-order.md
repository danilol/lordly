...
**Zero draws, by design:** the Wizard/Sorceress row blast (magic — neither dodges nor crits, per target or otherwise), heals, Witch status casts, Guard (story 4.7, REVISED — supersedes the redirect design below: a post-pipeline Full/Half damage reduction, zero draws, the protected unit stays the target so A3/A4 resolve against it exactly as normal — see the dossier §4 amendment), leader-fall penalty (sober package — deterministic reversion, no "panicked" draws), Golem two-cell resolution (deterministic), tactics target selection (ties fall back to Autonomous priority — FR34, no new randomness).

> **Amendment (stories 5.4/5.5, recorded 2026-08-01 at the 5.4 engine review):** the zero-draw list above named only "the Wizard/Sorceress row blast" because that was the era's only roll-free attack. Two kinds have joined it since, both under the SAME rule (A3/A4 exist only for a physical SINGLE-TARGET hit) and neither adding, removing, or reordering a draw:
>
> - `bolt` (story 5.4, dossier E5-D4) — magic, ranged, single-target. It replaced the casters' `blast` on every class that had one, so **no class carries `blast` today**; the kind and its arithmetic stay in the engine as reserved data (a future Archmage) and the `case 'blast'` branch stays live and data-driven.
> - `breath` (story 5.5, E5-D7) — the dragons' row-AoE. PHYSICAL arithmetic (STR vs VIT) but roll-free, which is the point worth writing down: the zero-draw property keys on **single-target vs row-AoE**, never on magic-vs-physical. A row-AoE has no single defender to key A3's dodge to.
>
> One CONSEQUENCE is a real behavioural change, recorded here so no future reader mistakes it for drift: a confused mid/back caster used to misfire as a self-blast, which picks its target by row rule and therefore drew **A1 only**. Its misfire is now a bolt at a random ally, so it draws **A1 + A2** — the A2 row's own condition ("A1 misfired AND a valid redirect target exists") always covered this; only the old self-blast sat outside it. Stored seeds are unaffected: the change rode `balanceVersion` 9 → 10, and history is replay-gated on `balanceVersion` (AD-8).

> **Amendment (story 4.7, 2026-07-19, Danilo):** the line above originally read "Guard interception (deterministic redirect — target finalization happens BEFORE A3/A4, so the dodge/crit roll against the GUARD as defender)" per the dossier's original column-bodyguard design (D-2a). That redirect was replaced before it shipped: Guard is now a one-shot Full/Half damage SHIELD, checked as the OUTERMOST post-pipeline step, AFTER `rollHit`'s A3/A4 already resolved against the actually-attacked unit. There is no retarget, so this ADR's frozen A1–A4 draw table is completely unaffected — this is a prose correction, not a resequencing.
