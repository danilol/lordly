...
const VIEW_TOP = 44;
const MARGIN = 16;
/** Compact unit-card metrics (DESIGN `{components.unit-card}`): side-colored border + wash, 32px sprite, 3-letter code, element dot. */
const CARD_W = 46;
const CARD_H = 56;
const CARD_GAP = 3;
...
export class HistoryScene extends Scene {
  ...
  create() {
    ...
    if (entries.length === 0) {
      ...
      for (const entry of entries) {
        y = this.renderRow(content, entry === entries[0] ? y : y + 12, formatHistoryRow(entry));
      }
    }

    const wasDrag = enableDragScroll(this, content, VIEW_TOP, y + 16, BASE_HEIGHT - 16);
  ...
  /** One match row: verdict + date header line, then the two compositions face to face. Returns the next free y. */
  private renderRow(content: GameObjects.Container, y: number, row: ReturnType<typeof formatHistoryRow>): number {
    ...
    const vs = crispText(this, x + 10, cardsY + CARD_H / 2, 'vs', { fontFamily: 'Arial', fontSize: '11px', color: PALETTE.mutedText }).setOrigin(0.5);
    content.add(vs);
    x += 20;
    ...
    // A hairline under the row; the right edge stays free for 3.2's Replay.
